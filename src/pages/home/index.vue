<template>
    ...
    <van-row justify="space-around">
        <van-col span="11" class="center-img" v-for="(item, index) in data.nav2s" :key="item.id"
            @click="goOrderTwo(index)">
            <van-image radius="5" :src="item.pic_image_url" /></van-col>
    </van-row>
    <van-row justify="space-around" v-for="item in data.hospitals" :key="item.id" class="yy-list" @click="goOrderhospital(item)">
        <van-col span="6">
...
onMounted(() => {
    indexIndex().then((result) => {
        //console.log(result)
        Object.assign(data, result.data.data)
        console.log(data)
    })
})
const goOrderTwo = function (index) {
    console.log(data.hospitals[index].id)
    router.push(`/createOrder?id=${data.hospitals[index].id}`)
}
const goOrderhospital=function(item){
