<template>
    <div class="container">
        <div class="header">
            <van-icon @click="goBack" name="arrow-left" class="header-left" size="30" />
            填写服务订单
        </div>
        <statusBar item="0"></statusBar>
        <van-cell value="内容" class="cell">
            <template #title>
                <van-image :src="Data.service.serviceImg" width="25" height="25">
                </van-image>
                <span class="server-name">{{ Data.service.serviceName }}</span>
            </template>
            <template #default>
                <div class="server-text">服务内容</div>
            </template>
        </van-cell>
        <van-cell-group class="cell">
            <van-cell title="就诊医院" :value="formData.hospital_name" is-link @click="showHospital">
            </van-cell>
            <van-cell title="就诊时间" is-link @click="showTime">
                <template #value>
                    {{ date || '请选择就诊时间' }}
                </template>
            </van-cell>
            <van-cell title="陪诊师" is-link @click="showCompanion">
                <template #value>
                    {{ companionName || '请选择陪诊师' }}
                </template>
            </van-cell>
            <van-cell>
                <template #title>
                    接送地址
                </template>
                <van-field class="text" input-align="right" v-model="formData.receiveAddress" placeholder="请输入接送地址" />
            </van-cell>
            <van-cell>
                <template #title>
                    联系电话
                </template>
                <van-field class="text" input-align="right" v-model="formData.tel" placeholder="请输入联系电话" />
            </van-cell>
        </van-cell-group>
        <van-cell-group title="服务需求" class="cell">
            <van-field class="text" v-model="formData.demand" style="height: 100px;" placeholder="请简单描述您要就诊的科室" />
        </van-cell-group>

        <van-dialog v-model:show="showQrCodeImage" :show-confirm-button="false">
            <van-icon name="cross" class="close" @click="closeCode" />
            <div>微信支付</div>
            <van-image :src="qrCodeImage" width="150" height="150"></van-image>
            <div>请使用本人微信扫描二维码</div>
        </van-dialog>

        <van-button type="primary" block @click="sumbitForm">提交订单</van-button>
        <van-popup v-model:show="showHospitalPop" position="bottom">
            <van-picker :columns="columns" @confirm="onConfirm" @cancel="showHospitalPop = false" />
        </van-popup>

        <van-popup v-model:show="showTimePop" position="bottom">
            <van-date-picker @confirm="onConfirmdata" @cancel="showTimePop = false" title="选择年月" :min-date="minDate"
                :columns-type="columnsType" />
        </van-popup>

        <van-popup v-model:show="showCompanionPop" position="bottom">
            <van-picker :columns="companionColumns" @confirm="onConfirmCompanion" @cancel="showCompanionPop = false" />
        </van-popup>
    </div>



</template>

<script setup>
import { onMounted, reactive, ref } from 'vue'
import { useRouter, useRoute } from 'vue-router'
import statusBar from '../../components/statusBar.vue'
import { h5Companion, CreateOrder } from '../../api'
import Qrcode from 'qrcode'
const showQrCodeImage = ref(false)
const formData = reactive({
    hospital_id: '',
    hospital_name: '',
    demand: '',
    companion_id: 0,
    receiveAddress: '',
    tel: '',
    starttime: 0
})
const qrCodeImage = ref('')
const closeCode=function(){
    showQrCodeImage.value=false
    router.push('/order')
}
const sumbitForm = function () {
    for (let item in formData) {
        //console.log(formData[item])
        if (!formData[item]) {
            //console.log('111')
            showNotify({ message: '请把每一项都填写' })
            return
        }
    }
    CreateOrder(formData).then((result) => {
        Qrcode.toDataURL(result.data.data.wx_code).then((url) => {
            qrCodeImage.value = url
            showQrCodeImage.value = true
        })
        //console.log(qrCodeImage.value)
    })
}
const onConfirmCompanion = function (selectedValues) {
    console.log(selectedValues)
    formData.companion_id = selectedValues.selectedValues[0]
    companionName.value = selectedValues.selectedOptions[0].text
    showCompanionPop.value = false
}
const showCompanionPop = ref(false)
const showTimePop = ref(false)
const columnsType = ['year', 'month', 'day']
const minDate = new Date(new Date().getFullYear(), new Date().getMonth(), new Date().getDate())
const columns = ref([])
const companionColumns = ref([])
const onConfirm = ({ selectedValues }) => {
    columns.value.forEach(item => {
        if (item.value === selectedValues[0]) {
            formData.hospital_name = item.text
            formData.hospital_id = selectedValues[0]
            //console.log(formData.hospital_id,formData.hospital_name)
            //console.log(selectedValues[0],item.value)
        }
    })
    showHospitalPop.value = false
}
const date = ref('')
const companionName = ref('')
const onConfirmdata = function (selectedValues) {
    console.log(selectedValues)
    const dateStr = selectedValues.selectedValues.join('-')
    date.value = dateStr
    formData.starttime = new Date(dateStr).getTime()
    showTimePop.value = false
}
const showHospitalPop = ref(false)
const showHospital = () => {
    showHospitalPop.value = true
}
const showTime = function () {
    showTimePop.value = true
}
const showCompanion = function () {
    showCompanionPop.value = true
}
onMounted(() => {
    h5Companion().then((result) => {
        Object.assign(Data, result.data.data)
        console.log(Data)
        Data.hospitals.forEach((item, index) => {
            columns.value[index] = { text: item.name, value: item.id }
            if (item.id === +(route.query.id)) {
                formData.hospital_name = item.name
                formData.hospital_id = item.id
            }
            //console.log(columns.value)
            //console.log(hospitalData) 
        })
        Data.companion.forEach((item, index) => {
            companionColumns.value[index] = { text: item.name, value: item.id }
        })
    })
})
const route = useRoute()
const Data = reactive({
    companion: [],
    hospitals: [],
    service: {}
})
const router = useRouter()
const goBack = function () {
    router.go(-1)
}
</script>

<style lang="less" scoped>
.container {
    background-color: #f0f0f0;
    height: 110vh;
}

.header {
    padding: 10px 0;
    text-align: center;
    line-height: 30px;
    font-size: 15px;

    .header-left {
        float: left;
    }
}

.cell {
    width: 95%;
    margin: 5px auto;
    background-color: #fff;

    ::v-deep(.van-field__control) {
        color: var(--van-cell-value-color);
    }

    ::v-deep(.van-cell__title) {
        display: flex;
        align-items: center;
    }

    .server-name {
        margin-left: 10px;
    }
}

.service-text {
    background: url(data:image/png;base64,) no-repeat center center;
    background-size: 20px;
}

.sumbit {
    position: absolute;
    bottom: 0;
}

::v-deep(.van-dialog__content) {
    text-align: center;
    padding: 20px;

    .close {
        position: absolute;
        left: 20px;
    }
}
</style>