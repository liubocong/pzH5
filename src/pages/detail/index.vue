<template>
  ...
  <div class="tips">
    <div class="dzf" v-if="detailData.trade_state === '待支付'">
      ...
      <div class="text2">请在
        <countDown :countdown="detailData.order_start_time"></countDown>
        内完成支付，超时订单自动取消
      </div>
      <div class="text3">
        <van-button type="success" @click="showQrCodeImage = true">立即支付（10元）</van-button>
      </div>
    </div>
...
onMounted(() => {
  orderDetail({ oid: route.query.oid }).then((result) => {
    Object.assign(detailData, result.data.data)
